...
namespace Databox.Ed
{
	public class EditorGUIHierarchyView {
		...
		DataboxObject lastSelectedDataboxObject;
		...
		public bool BeginNode(string label) {
			return Node (label, true, defaultUnselectedTextColor,defaultSelectedTextColor, null);
		}
	
		//Returns true if this node is selected
		public bool BeginNode(string label, Color unselectedTextColor, Color selectedTextColor) {
			return Node (label, true,unselectedTextColor,selectedTextColor, null);
		}
		...
		public bool Node(string label, Color unselectedTextColor, Color selectedTextColor) {
			...
			return Node (label, false, unselectedTextColor,selectedTextColor, null);
		}
	
		//Returns true if this node is selected
		public bool Node(string label, Databox.DataboxObject _databox) {
	
			return Node (label, false, defaultUnselectedTextColor,defaultSelectedTextColor, _databox);
		}
	
		bool Node(string label, bool isParent, Color unselectedTextColor, Color selectedTextColor, Databox.DataboxObject _databoxObject) {
			...
			if (isParent) {
				...
				styleToUse.onFocused.textColor = colorToUse;
					...
					opened = EditorGUILayout.Foldout (wasOpened, label, true, styleToUse);
				...
				if (_databoxObject.errors == Databox.DataboxObject.ErrorType.UnsavedChanges)
				{
					styleToUse.fontStyle = FontStyle.Bold;
					EditorGUILayout.LabelField(label + "*", styleToUse );
				}
				else
				{
					styleToUse.fontStyle = FontStyle.Normal;
					EditorGUILayout.LabelField ( label, styleToUse);
				}
				
				GUILayout.FlexibleSpace();
				
				if (GUILayout.Button(EditorGUIUtility.IconContent("d_Search Icon"), GUILayout.Width(30), GUILayout.Height(20)))
				{
					EditorGUIUtility.PingObject(_databoxObject);
				}
